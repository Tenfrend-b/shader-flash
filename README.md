...
作者：**Tenfrend_b**（Tenfrend_b@outlook.com）· 许可证：**LGPL-3.0-or-later** · 目标版本：Minecraft **1.20.4**（Fabric，仅客户端）
...
外加两个包材质表不同时的一次区块重建 —— 实测从数秒降到**几十毫秒**。
...
可以像普通光影包一样被切换、被截图、被自检（想完全原生请在 Iris 里关掉光影）。
...
* **兼容性自检**：对比两条管线的材质 ID 表、AO 等级、体素化光源、实体分离绘制、阴影分辨率、
  各自自带的程序，并报告**着色器程序链接状态**（驱动资源不足时会明确告诉你，而不是把坏管线交给你用）。

### 5. 快捷键（可在“选项 → 控制 → 闪光灯”里改）

| 按键 | 作用 |
| --- | --- |
| **F7** | 主/副管线秒切（还没有副管线时会就地构建） |
| **F6** | 打开/关闭双管线 |
| **F8** | 打开设置界面 |
| **F2** | 副管线截图（需在设置里开启） |
